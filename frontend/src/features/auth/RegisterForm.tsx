    ...
    <div style={{
      ...
      <div>
        ...
        <p style={{ opacity: 0.8, lineHeight: 1.65, fontSize: '0.95rem' }}>
          Monitor and report on nature-based solutions with scientific-grade geospatial tools, built for conservation professionals.
        </p>
      </div>
      <p style={{ opacity: 0.5, fontSize: '0.75rem' }}>© 2024 Darukaa.Earth. All rights reserved.</p>
    </div>
...
export const RegisterForm: React.FC = () => {
  ...
  return (
    <AuthLayout>
      <h1 style={{ fontSize: '1.625rem', fontWeight: 700, color: 'var(--color-text)', marginBottom: '0.25rem' }}>Create your account</h1>
      <p style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)', marginBottom: '2rem' }}>Start tracking environmental impact today</p>

      <form onSubmit={handleSubmit} noValidate>
        <div className="field">
          <label className="field__label" htmlFor="reg-name">Full name</label>
          <input id="reg-name" className="field__input" type="text" autoComplete="name" placeholder="Jane Smith" value={fullName} onChange={e => setFullName(e.target.value)} />
        </div>
        <div className="field">
          <label className="field__label field__label--required" htmlFor="reg-email">Email address</label>
          <input id="reg-email" className="field__input" type="email" autoComplete="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} required />
        </div>
        <div className="field">
          <label className="field__label field__label--required" htmlFor="reg-password">Password</label>
