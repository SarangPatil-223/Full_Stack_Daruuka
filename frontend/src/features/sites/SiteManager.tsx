...
import { getSites, createSite, deleteSite, type Site, type SiteCreate } from '../../services/sitesApi';
...
export const SiteManager: React.FC = () => {
  ...
  const handlePolygonDrawn = (geojson: GeoJSON.Polygon, areaM2: number) => {
  ...
  const handleSaveSite = async () => {
    if (!siteName.trim()) { setSaveMsg('Please enter a site name.'); return; }
    if (!drawnGeometry) { setSaveMsg('Please draw a polygon on the map first.'); return; }
    try {
      setIsSaving(true);
      setSaveMsg('');
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div>
          <Link to="/projects" style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)' }}>
            ← Back to Projects
          </Link>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 600, marginTop: '0.25rem' }}>Site Manager</h1>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        {drawnGeometry && drawnAreaM2 !== null && (
          <div style={{
            ...
            ✅ Polygon drawn — approx. <strong>{(drawnAreaM2 / 10000).toFixed(2)} ha</strong>. Fill in the form below and click Save.
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        {saveMsg && (
          <p style={{ marginTop: '0.75rem', fontSize: '0.875rem', color: saveMsg.startsWith('✅') ? 'var(--color-healthy)' : 'var(--color-critical)' }}>
            {saveMsg}
          </p>
        )}
      </div>
